...
The tree report renders an interactive tree and can include sample metadata, run
logs, and a custom title. It is useful after running `snippy-ng multi` and
`snippy-ng tree`, or any time you already have a Newick tree that you want to
inspect in a browser.

![Snippy-NG tree report showing an interactive tree and sample metadata](tree.png)
...
| `--prefix`, `-p` | `tree` | Prefix for the generated HTML file. |