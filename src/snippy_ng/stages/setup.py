...
class NcbiGenbankReferenceOutput(BaseOutput):
    genbank: Path = Field(..., description="Downloaded NCBI GenBank flat-file reference")


class DownloadNcbiGenbankReference(BaseStage):
    ...
    @property
    def output(self) -> NcbiGenbankReferenceOutput:
        output_directory = self.output_directory or Path(".")
        return NcbiGenbankReferenceOutput(
            genbank=output_directory / f"{self.reference_prefix}.gbff",
        )

    def create_commands(self, ctx):
        return [
            self.python_cmd(
                func=self.download_genbank_reference,
                args=(self.accession, self.output.genbank),
                description=f"Download NCBI GenBank reference: {self.accession}",
            ),
        ]

    def download_genbank_reference(
        self,
        accession: str,
        output_genbank_path: Path,
    ) -> None:
        output_genbank_path.parent.mkdir(parents=True, exist_ok=True)
        ...
        with tempfile.TemporaryDirectory(prefix=f"snippy-ng-{accession}-") as tmpdir:
            ...
            try:
                ...
                raise StageExecutionError(
                    f"NCBI Datasets API could not download assembly '{accession}' "
                    f"(HTTP {exc.code}). Check the accession and try again."
                ) from exc
            except urllib.error.URLError as exc:
                raise StageExecutionError(
                    f"Could not reach NCBI Datasets API while downloading assembly '{accession}': {exc.reason}"
                ) from exc

            try:
                with zipfile.ZipFile(zip_path) as archive:
                    genbank_member = self._find_archive_member(archive, "_genomic.gbff")
                    if genbank_member is None:
                        genbank_member = self._find_archive_member(archive, ".gbff")
                    if genbank_member is None:
                        raise StageExecutionError(f"NCBI assembly package for '{accession}' did not contain a GenBank flat file.")
                    with archive.open(genbank_member) as source, open(output_genbank_path, "wb") as target:
                        shutil.copyfileobj(source, target)
                    report = self._load_assembly_report(archive)
            except zipfile.BadZipFile as exc:
                raise StageExecutionError(f"NCBI returned an invalid zip package for assembly '{accession}'.") from exc

        assembly_name = report.get("assembly_info", {}).get("assembly_name") if report else None
        suffix = f" ({assembly_name})" if assembly_name else ""
        print(f"Downloaded NCBI GenBank assembly {accession}{suffix} to {output_genbank_path}")
    ...
    def _load_assembly_report(archive: zipfile.ZipFile) -> dict:
        member = DownloadNcbiGenbankReference._find_archive_member(archive, "assembly_data_report.jsonl")
        if member is None:
            return {}
    ...
    def create_commands(self, ctx):
        return [
            self.python_cmd(
                ...
                description=f"Download AllTheBacteria assembly reference: {self.accession}",
            ),
        ]
