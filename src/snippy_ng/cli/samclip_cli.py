...
@click.argument("sam_file", required=False, nargs=1, type=click.Path(exists=True, readable=True, path_type=AbsolutePath))
@click.option("--index", "-i", required=True, type=click.Path(exists=True, readable=True, path_type=AbsolutePath), help="Reference FASTA index (.fai) file corresponding to the reference used for alignment")
