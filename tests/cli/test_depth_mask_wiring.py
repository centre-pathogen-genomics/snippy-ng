...
def test_short_pipeline_uses_combined_depth_beds(tmp_path):
    ...
    depth_mask = _stage_instances(ApplyDepthMaskToFasta)[0]
    ...
    assert depth_beds.min_base_quality == 13
    assert depth_beds.min_mapping_quality == 60
    assert _stage_types().index(BcftoolsPseudoAlignment) < _stage_types().index(ApplyDepthMaskToFasta)


def test_short_pipeline_threads_min_mapping_quality_to_caller_and_depth_beds(tmp_path):
    paths = {
        "ref": tmp_path / "ref.fa",
        "r1": tmp_path / "reads_1.fq",
        "r2": tmp_path / "reads_2.fq",
...
def test_long_pipeline_threads_min_mapping_quality_to_freebayes_and_depth_beds(tmp_path):
    paths = {
        "ref": tmp_path / "ref.fa",
        "reads": tmp_path / "reads.fq",
        "out": tmp_path / "output",
